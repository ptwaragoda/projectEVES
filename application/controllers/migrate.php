...
class Migrate extends CI_Controller
{
	...
	function install()
	{
		if(!$this->migrations->install()) show_error($this->migrations->error);
	...
	{
		//if(!$this->tank_auth->is_logged_in()) show_error('You do not have permission to access this');
		//show_error('Access to this controller is blocked, turn me on when you need me.');
		// No $id supplied? Use the config version
		$id OR $id = $this->config->item('migrations_version');

		if(!$this->migrations->version($id)) show_error($this->migrations->error);
		echo "<br />Migration Successful<br />";
	}
}
